...
class Value {
    ...
    fromDecimal(value) {
        ...
        const part = value
            .valueOf()
            .match(/(?<mantissaSign>\+|-)?(?<integer>\d+)(?:\.)?(?<fraction>\d+)?(?:e|E)?(?<exponentSign>\+|-)?(?<exponent>\d+)?/).groups;
        const defined = (item, no, yes) => item === undefined ? no : yes(item);
    ...
    toDecimal() {
        ...
        const exponent = `${this._exponent.length !== 0 ? 'e' : ''}${this._exponentSign}${this._exponent}`;
...
export var Processor = (_a = class Processor {
        constructor() {
            Decimal.Decimal.set({
        ...
        _setPoint() {
            ...
            else if (this._number.isInteger())
        ...
        _setDigit(lambda) {
            ...
            if (this._isMode(Processor.Mode.EXPONENT)) {
                ...
                const exponent = parseInt(this._number.exponent) +
                    parseInt(value.exponent.length === 0 ? '0' : value.exponent);
                if (exponent > Processor.Precision.MAX_E_VALUE ||
